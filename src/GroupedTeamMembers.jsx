...
const GroupedTeamMembers=({employees, selectedTeam,setTeam})=>{
    ...
    return(
        <main className="container">
           {
            groupedEmployees.map((grouped)=>{
                return (
                    <div key={grouped.team} className="card mt-3" style={{cursor:'pointer'}}>
                        <h4 id={grouped.team} className="card-header text-secondary bg-white" onClick={handleTeamClick}>
                                Team Name : {grouped.team}
                        </h4>
                        <div id={"collapse_" + grouped.team} className={grouped.collapsed === true ? "collapse":""}>
                            <hr/>
                            {
                                grouped.members.map((member)=>{
                                    return (
                                        <div className="mt-2">
